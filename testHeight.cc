/* 5G NR Drone Rescue Simulation - Improved for Altitude Impact
 ...
 * higher TxPower, fixed seed, longer simTime, sub-6 GHz band with UMa scenario
 */
 ...
 NS_LOG_COMPONENT_DEFINE("DroneRescueAltitudeImproved");
 ...
 main(int argc, char *argv[])
 {
   ...
   std::ofstream csv("results_height.csv");
   ...
   for (double height : droneHeights)
   {
     ...
     nrHelper->SetGnbBeamManagerAttribute("UpdateDelay", TimeValue(MilliSeconds(5)));  // Frequent update for mobility/altitude
     ...
     // Higher numerology for better tolerance
     nrHelper->SetGnbPhyAttribute("Numerology", UintegerValue(3));
 
     /* ================= BAND (sub-6 3.5 GHz with UMa) ================= */
     CcBwpCreator ccBwpCreator;
     CcBwpCreator::SimpleOperationBandConf bandConf(
         3.5e9,
         40e6,
         1,
         BandwidthPartInfo::UMa);  // UMa for wider coverage
     ...
     std::ofstream out("results_height.csv", std::ios::app);
     out << height << ","
         << avgDelay << ","
         << avgJitter << ","
         << loss << ","
   ...
   std::cout << "\nSimulation completed. Results saved to results_height.csv\n";
   return 0;
 }